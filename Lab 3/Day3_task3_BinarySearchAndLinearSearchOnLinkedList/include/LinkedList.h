...
public:
    ...
    int linearSearch(int target)
    {
        ...
        while(current!=NULL)
        {
            ...
            {
                cout<<"number of iterations for linear search on linked list ="<<iterations<<endl;
                return index;
            }
            current=current->next;
            index++;
        }
        cout<<"total number of iterations for linear search on linked list ="<<iterations<<endl;
        return -1;
    }
    ...
    {
        int start=0,end=counter-1,mid=0,iterations=0;
        while(start<=end)
        {
            iterations++;
            mid=(start+end)/2;
            int data=GetDataByIndex(mid);
            Node* middleNode=getNodeByData(data);
            ...
            else if(middleNode->data > search)
            {
                end=mid-1;
            }
            else if(middleNode->data < search)
